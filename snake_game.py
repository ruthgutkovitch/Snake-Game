...
class Screen:
    ...
    def endGame(self,screen_width,screen_height):
        len = self.player.len
        for i in range(len):
        ...
        while(self._running):
            for event in pygame.event.get():
                if event.type==pygame.QUIT:
                    self._running=False
                    self.printMessage(" You Ended The Game")
            ...
            if self.endGame(self.screen_width,self.screen_height)==True:
                self._running=False
                self.printMessage("You lost ")

            self.displayScreen()
            if self.apple.food_x==self.player.x[-1] and self.apple.food_y==self.player.y[-1]:
                self.score += 1
                self.player.len += 1
